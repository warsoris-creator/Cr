...
async def cb_bot_card(callback: types.CallbackQuery):
    ...
    status_text = "Running" if status == "active" else "Stopped" if status == "inactive" else "Error"
...
async def cb_start(callback: types.CallbackQuery):
    ...
    await callback.answer(f"▶️ Запущен: {db_status}")
...
async def cb_restart(callback: types.CallbackQuery):
    ...
    db_status = "running" if status == "active" else "error"
    await db.update_bot_status(bot_id, db_status)
    await callback.answer(f"🔄 Перезапущен: {db_status}")
...
async def cb_update(callback: types.CallbackQuery):
    ...
    if not bot or bot["source_type"] != "github":
        await callback.answer("⚠️ Обновление доступно только для GitHub ботов")
        return

    await callback.message.answer("⏳ Обновляю код из GitHub...")
    import subprocess
    try:
        subprocess.run(["sudo", "git", "-C", bot["work_dir"], "pull"],
                      check=True, capture_output=True, timeout=120)
        await deploy.restart_service_by_name(bot["systemd_unit"])
        await callback.message.answer("✅ Обновлено и перезапущено")
